...
/**
 * Shared fixture and test scenario for the upstream {@code volumes-emptydir} example.
 *
 * <p>Like {@link EmptyDirVolumeBase}, the {@code emptyDir} code path is DinD-compatible —
 * the daemon receiving a bind-mount of an empty local path produces the correct empty
 * directory. Both the local and DinD variants are expected to pass.
 */
public abstract class VolumesEmptyDirBase {
     ...
     * {@link EmptyDirVolumeBase} using a POSIX-safe fixture.
     */
    @Test
    void emptyDirVolumeIsMounted() throws Exception {
        ...
        try (WorkflowRun run = configure(ArgoWorkflowExecutor.from(wf))
