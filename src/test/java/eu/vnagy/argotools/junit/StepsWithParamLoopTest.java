...
class StepsWithParamLoopTest {
    ...
    void withItemsExpandsIntoMultipleIterations() throws Exception {
        try (var run = ArgoWorkflowExecutor
                .from(Path.of(getClass().getResource("/steps-withitems-loop.yaml").toURI()))
                .execute(Duration.ofMinutes(10))) {

            assertThat("workflow succeeded", run.succeeded(), is(true));

            StepsRun steps = (StepsRun) run.entrypoint();
            List<WorkflowNode> iterations = steps.get("echo-each").children();
